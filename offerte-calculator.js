(function(){
  ...
  var typeCards = document.querySelectorAll('#typeCardGrid .type-card');
  var gaasCards = document.querySelectorAll('#gaasCardGrid .type-card');
  var gaasDescriptionEl = document.getElementById('gaasDescription');
  var lengteEl = document.getElementById('lengte');
  var breedteEl = document.getElementById('breedte');
  var naamEl = document.getElementById('naam');
  var emailEl = document.getElementById('email');
  var telefoonEl = document.getElementById('telefoon');

  var optRalEl = document.getElementById('optRal');
  var optRalWrap = document.getElementById('optRalWrap');
  var ralCodeWrap = document.getElementById('ralCodeWrap');
  var ralSwatches = document.querySelectorAll('.ral-swatch');
  var specGaasRow = document.getElementById('specGaasRow');
  var specGaasEl = document.getElementById('specGaas');
  var specGaasPriceRow = document.getElementById('specGaasPriceRow');
  var specGaasPriceLabel = document.getElementById('specGaasPriceLabel');
  var specGaasPrice = document.getElementById('specGaasPrice');
  var specRalRow = document.getElementById('specRalRow');
  var deliveryWindowEl = document.getElementById('deliveryWindow');
  var step2NextBtn = document.getElementById('step2NextBtn');

  var errLengte = document.getElementById('errLengte');
  var errBreedte = document.getElementById('errBreedte');
  var errNaam = document.getElementById('errNaam');
  var errEmail = document.getElementById('errEmail');
  var errTelefoon = document.getElementById('errTelefoon');

  var calcPriceEl = document.getElementById('calcPrice');
  var specType = document.getElementById('specType');
  var specDim = document.getElementById('specDim');
  var specArea = document.getElementById('specArea');
  var akkoordBtn = document.getElementById('akkoordBtn');
  ...
  function getSelectedType(){
    var checked = form.querySelector('input[name="typeHor"]:checked');
    return checked ? checked.value : 'raamhor';
  }

  var gaasSectionWrap = document.getElementById('gaasSectionWrap');
  var ralPriceLabel = document.getElementById('ralPriceLabel');

  function updateProductTypeUI(){
    var onRequest = !!ON_REQUEST_TYPES[getSelectedType()];
    var hasGaasChoice = !NO_GAAS_TYPES[getSelectedType()];
    /* Gaas (insectengaas) is niet van toepassing op shutters */
    gaasSectionWrap.style.display = hasGaasChoice ? 'block' : 'none';
    ralPriceLabel.style.display = onRequest ? 'none' : 'inline';
    specGaasRow.style.display = hasGaasChoice ? 'flex' : 'none';
  }
  ...
  /* Pre-select type from ?type= query param (links from the productoverzicht) */
  var params = new URLSearchParams(location.search);
  var typeParam = params.get('type');
  if(typeParam && TYPE_LABELS[typeParam]){
    typeCards.forEach(function(card){
      var input = card.querySelector('input');
      var isMatch = input.value === typeParam;
  ...
  typeCards.forEach(function(card){
    card.addEventListener('click', function(){
      typeCards.forEach(function(c){ c.classList.remove('is-checked'); });
      card.classList.add('is-checked');
      if(currentStep === 1){
  ...
  var wizardProgressFill = document.getElementById('wizardProgressFill');
  var wizardStepNumEl = document.getElementById('wizardStepNum');
  var wizardStepNameEl = document.getElementById('wizardStepName');
  var WIZARD_STEP_NAMES = {1:'Type hor', 2:'Afmetingen', 3:'Extra opties', 4:'Uw gegevens'};
  var currentStep = 1;

  function scrollWizardIntoView(){
    var wrap = document.querySelector('.calculator-wrap');
    if(!wrap) return;
  ...
  function goToStep(n){
    ...
    var activeLabel = document.querySelector('.wizard-step[data-step="' + n + '"] .calc-step-label');
    if(activeLabel){
  ...
  function updateCalculation(){
    ...
    step2NextBtn.disabled = !dimsValid;
    ...
    if(optRalEl.checked){
      var ral = getSelectedRal();
      var ralPriceSpec = getRalPrice();
      specRalRow.querySelector('span:last-child').textContent = (ral ? ral.name : '—') + (onRequest ? '' : (ralPriceSpec > 0 ? ' (+ ' + formatEUR(ralPriceSpec) + ')' : ' (inbegrepen)'));
    }
  ...
  function genereerPDF(){
    ...
    /* Watermerk: groot, vaag logo gecentreerd op de pagina, getekend
       voordat de rest van de inhoud erover heen komt. */
    ...
    var onRequestPdf = !!ON_REQUEST_TYPES[getSelectedType()];
    var hasGaasChoicePdf = !NO_GAAS_TYPES[getSelectedType()];

    /* Extra opties (transparant getoond met eigen prijs) */
    var extras = [];
    if(hasGaasChoicePdf){
  ...
  function verstuurOfferteMailFallback(){
    var onRequestMail = !!ON_REQUEST_TYPES[getSelectedType()];
    var hasGaasChoiceMail = !NO_GAAS_TYPES[getSelectedType()];
    var extras = [];
    if(hasGaasChoiceMail){
    ...
    var body = [
      ...
      onRequestMail ? 'Prijs: op aanvraag' : 'Indicatieve totaalprijs: ' + formatEUR(currentTotal),
      '',
      'Naam: ' + naam,
  ...
  function verstuurOfferteAanvraag(doc, offerteNummer){
    ...
    var onRequestSend = !!ON_REQUEST_TYPES[getSelectedType()];
    var hasGaasChoiceSend = !NO_GAAS_TYPES[getSelectedType()];
    var gaas = hasGaasChoiceSend ? getSelectedGaas() : null;
    ...
    var payload = {
      ...
      prijsLabel: onRequestSend ? 'Op aanvraag' : formatEUR(currentTotal),
