...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .swatch-std{background:#fff}
  .kleur-naam{font-size:12px;color:#a8a29e}

  .hor-preview-wrap{display:flex;flex-direction:column;align-items:center;gap:8px;margin-top:16px;padding-top:16px;border-top:1px dashed var(--line)}
  .hor-preview{width:170px;height:130px;position:relative;transform:perspective(700px) rotateY(-14deg) rotateX(4deg);transform-style:preserve-3d;transition:width .2s ease,height .2s ease}
  .hor-preview-front{position:absolute;inset:0;border-radius:3px;border:7px solid #8F8F8C;box-shadow:10px 16px 28px rgba(32,30,26,.20);background-color:#eef0ec;background-repeat:repeat;transition:border-color .2s,background-color .2s}
  .hor-preview-edge{position:absolute;top:0;right:-12px;width:12px;height:100%;background:#6b6b69;transform-origin:left center;transform:rotateY(90deg);transition:background .2s}
  .hor-preview-caption{font-size:12px;color:#a8a29e}
  ...
  .inkoop-row{display:flex;justify-content:space-between;align-items:center;font-size:14px;padding:7px 0;border-top:1px solid var(--line-soft)}
  .inkoop-row:first-child{border-top:none}
  .inkoop-row .l{color:var(--muted)}
  .inkoop-row .v{font-weight:500;color:var(--ink)}
  .inkoop-row.strong{margin-top:2px}
  .inkoop-row.strong .l,.inkoop-row.strong .v{font-weight:600;color:var(--ink)}
      ...
      <section class="card">
        <div class="opt-row" style="border-bottom:none;padding-bottom:0">
          <div class="opt-l">
            ...
            <div><div class="opt-t">Eigen eindprijs</div><div class="opt-s" id="eigenPrijsSub">Uit — gebruikt de berekende prijs</div></div>
          </div>
          <button class="toggle" id="eigenPrijsToggle"><span class="knob"></span></button>
        </div>
        <div id="eigenPrijsBody" style="display:none;padding-top:16px">
          <div class="profiel-toggle" style="margin-bottom:12px">
            <button type="button" class="ptbtn" id="epExcl">Bedrag excl. BTW</button>
            <button type="button" class="ptbtn" id="epIncl">Bedrag incl. BTW</button>
          </div>
    ...
    <div>
      <div class="sticky">
        <div class="panel">
          ...
          <div class="panel-tot"><span class="l" id="ovTotaalLabel">Totaal (excl. BTW)</span><span class="v" id="ovTotaal">€ 0,00</span></div>
        </div>
...
var RAL_KLEUREN=[
  ...
  {code:"RAL9010",naam:"Zuiverwit",hex:"#F4F4F1"},
  {code:"RAL9005",naam:"Gitzwart",hex:"#0A0A0A"},
  {code:"RAL7016",naam:"Antracietgrijs",hex:"#383E42"},
  {code:"RAL7021",naam:"Zwartgrijs",hex:"#23282B"},
  {code:"RAL7040",naam:"Venstergrijs",hex:"#9DA3A6"},
  {code:"RAL9007",naam:"Grijs aluminium",hex:"#8F8F8C"},
  {code:"RAL8017",naam:"Chocoladebruin",hex:"#442F29"},
  {code:"RAL6009",naam:"Dennengroen",hex:"#27352A"},
  {code:"RAL5010",naam:"Gentiaanblauw",hex:"#17365D"}
];
function kleurHex(code){ var k=RAL_KLEUREN.find(function(x){return x.code===code;}); return k?k.hex:"#8F8F8C"; }
function kleurNaam(code){ var k=RAL_KLEUREN.find(function(x){return x.code===code;}); return k?k.naam:""; }
...
function isPlisseProduct(naam){ return /honeycomb|plisse/i.test(naam||""); }
function meshPattern(){
  return "repeating-linear-gradient(0deg, rgba(32,30,26,.08) 0 1px, transparent 1px 7px), repeating-linear-gradient(90deg, rgba(32,30,26,.08) 0 1px, transparent 1px 7px)";
}
function plissePattern(donker){
  var c=donker?"rgba(32,30,26,.22)":"rgba(32,30,26,.10)";
  return "repeating-linear-gradient(180deg, "+c+" 0 3px, transparent 3px 11px)";
}
function computePreviewSize(breedte,hoogte){
  var maxW=170,maxH=130,minPx=54;
  var w=parseFloat(breedte)||100, h=parseFloat(hoogte)||100;
  w=Math.max(20,Math.min(400,w)); h=Math.max(20,Math.min(400,h));
  var scale=Math.min(maxW/w,maxH/h);
  return {pw:Math.max(minPx,Math.round(w*scale)), ph:Math.max(minPx,Math.round(h*scale))};
}
...
function bereken(){
  var rows=state.regels.map(function(r){
    ...
    var effect=basis+((r.kleur||r.rall)?RALL:0);
  ...
  var berekendEind=eind;
  var epActief=!!(state.eigenPrijs&&state.eigenPrijs.actief);
  if(epActief){
    var epBedrag=parseFloat(state.eigenPrijs.bedrag);
    if(!isNaN(epBedrag)&&epBedrag>=0){
      if(state.eigenPrijs.basis==="incl"){
        var btwPctVoorEp=parseFloat(state.btwPct)||0;
        eind=epBedrag/(1+btwPctVoorEp/100);
      } else {
        eind=epBedrag;
      }
    }
...
function renderRegels(){
  ...
  state.regels.forEach(function(r,i){
    ...
    var swWrap=d.querySelector('.swatches'); var kleurLabelEl=d.querySelector('.kleur-naam');
    function setKleurLabel(){ kleurLabelEl.textContent = r.kleur ? (kleurNaam(r.kleur)+" +"+euro(RALL)+"/m²") : "Standaard"; }
    var activeKleur = r.kleur || (r.rall ? "RAL9005" : "");
    RAL_KLEUREN.forEach(function(k){
      var sw=document.createElement('button'); sw.type='button';
      sw.className='swatch'+(k.code===''?' swatch-std':'')+(activeKleur===k.code?' sel':'');
      sw.title=k.naam+(k.code?' ('+k.code+')':'');
      sw.style.background=k.hex;
      sw.onclick=function(){
        r.kleur=k.code; r.rall=!!k.code;
        swWrap.querySelectorAll('.swatch').forEach(function(s){s.classList.remove('sel');});
        sw.classList.add('sel');
        setKleurLabel();
        recompute();renderDoc();updatePreview(r);
      };
...
function syncEigenPrijsUI(){
  ...
  el("eigenPrijsBody").style.display=actief?"block":"none";
  el("epExcl").classList.toggle("sel", state.eigenPrijs.basis!=="incl");
  el("epIncl").classList.toggle("sel", state.eigenPrijs.basis==="incl");
  el("eigenPrijsBedrag").value=state.eigenPrijs.bedrag||"";
  el("eigenPrijsSub").textContent=actief?("Aan — bedrag "+(state.eigenPrijs.basis==="incl"?"incl.":"excl.")+" BTW"):"Uit — gebruikt de berekende prijs";
}
...
function initApp(){
  ...
  el("eigenPrijsToggle").onclick=function(){state.eigenPrijs.actief=!state.eigenPrijs.actief;syncEigenPrijsUI();recompute();renderDoc();};
  el("epExcl").onclick=function(){state.eigenPrijs.basis="excl";syncEigenPrijsUI();recompute();renderDoc();};
